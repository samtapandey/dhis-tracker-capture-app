...
trackerCapture.controller('SelectionController',
    function ($rootScope,
        ...
        $scope.eventsTodayFilters = [{
            name: $translate.instant('events_today_all'),
            value: 'all'
        }, {
            name: $translate.instant('events_today_completeoractive'),
            value: 'completedOrActive',
            status: ['COMPLETED', 'ACTIVE']
        }, {
            name: $translate.instant('events_today_skipped'),
            value: 'skipped',
            status: ['SKIPPED']
        }, {
            name: $translate.instant('events_today_scheduled'),
            value: 'scheduled',
            status: ['SCHEDULE']
        }];
        ...
        $scope.enrollment = {
            ...
            incidentStartDate: '',
            incidentEndDate: '',
            operator: $scope.defaultOperators[0]
        };
        $scope.searchMode = {
            listAll: 'LIST_ALL',
            freeText: 'FREE_TEXT',
        ...
        $scope.fetchTeis = function () {
            ...
            if ($scope.enrollmentStatus === 'TODAY') {
                ...
                CustomIdService.getChildren($scope.searchingOrgUnit.id).then(function (res1) {
                    if(res1.children.length == 0){
                        CustomIdService.getParentId($scope.searchingOrgUnit.id).then(function (res) {
                            $scope.parent = res.parent;
                            TEIService.search($scope.parent.id,
            ...
            var modalInstance = $modal.open({
                ...
                resolve: {
                    ...
                    hiddenGridColumns: function () {
                        return ($filter('filter')($scope.gridColumns, {
                            show: false
                        })).length;
                    },
