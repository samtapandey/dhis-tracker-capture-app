...
var trackerCaptureapexAEMS = angular.module('trackerCapture');
		trackerCaptureapexAEMS.controller('ApexQueueControllerAEMS', ["$rootScope", "$scope", "$timeout", "$location", "SessionStorageService", "CurrentSelection", function ($rootScope, $scope, $timeout, $location, SessionStorageService, CurrentSelection) {
				...
				$.getJSON('../api/sqlViews/TNEACle8fKx/data.json?var=orgunit:' + ouid2, function (response) {
					...
					for (var i = 0; i < response.rows.length; i++) {
						...
						events2[i].tei = response.rows[i][0];
													...
													if(response.rows[i][8] != ""){events2[i].sampleReceivedCSF  = "Yes"}
													else{events2[i].sampleReceivedCSF  = "No"}
													if(response.rows[i][9] != ""){events2[i].sampleReceivedSERUM  = "Yes"}
													else{events2[i].sampleReceivedSERUM  = "No"}
													if(response.rows[i][10] != ""){events2[i].sampleReceivedWB  = "Yes"}
													else{events2[i].sampleReceivedWB  = "No"}
